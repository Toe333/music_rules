...
def _scale(
    number: int, name: str, degrees: list[int] | None, notes: str, status: ScaleStatus
) -> Scale:
    return {
...
_RAW: Final[tuple[Scale, ...]] = (
    ...
    _scale(
        4,
        "Lydian Dominant (Overtone scale 1, 2, 3, #4, 5, 6, ♭7)",
        [0, 2, 4, 6, 7, 9, 10],
    ...
    _scale(
        6,
        "Mixolydian (Dominant 7 chord scale)",
        [0, 2, 4, 5, 7, 9, 10],
    ...
    _scale(
        7,
        "Aeolian (Natural Minor, 4th-chord scale)",
        [0, 2, 3, 5, 7, 8, 10],
    ...
    _scale(
        10,
        "Half-Whole Diminished (Dominant 7♭9 scale)",
        [0, 1, 3, 4, 6, 7, 9, 10],
    ...
    _scale(
        11,
        "Whole Tone (4-part 4th-chord scale)",
        [0, 2, 4, 6, 8, 10],
    ...
    _scale(
        12,
        "Whole-Half Diminished (4-part 4th-chord scale)",
        [0, 2, 3, 5, 6, 8, 9, 11],
    ...
    _scale(
        13,
        "Altered (Super-Locrian) — Dominant 7alt scale",
        [0, 1, 3, 4, 6, 8, 10],
    ...
    _scale(
        16,
        "Phrygian Dominant (Spanish minor)",
        [0, 1, 4, 5, 7, 8, 10],
...
def get_scale(scale_id: str) -> Scale:
    ...
    try:
        ...
        raise KeyError(
            f"Unknown scale id: {scale_id!r}. Try one of: {', '.join(sorted(SCALES))}."
        ) from exc
